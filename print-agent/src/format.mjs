...
export function formatKitchen(payload) {
  const lines = [
    'COZINHA / BAR',
    LINE,
    payload.tab || 'Comanda',
...
export function formatCustomer(payload) {
  const lines = [
    payload.store_name || 'Kabanas',
    LINE,
    payload.tab || 'Comanda',
  ];
  ...
  if (payload.service_amount) lines.push(`Serviço (${payload.service_rate || 0}%)${' '.repeat(8)}${money(payload.service_amount)}`);
